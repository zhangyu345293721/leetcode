...
class Solution:
    def diameter_of_binary_tree(self, root: TreeNode) -> int:
        ...
        if not root:
            return 0
        self.search_helper(root)
        return max_deep

    def search_helper(self, root: TreeNode) -> int:
        '''
            求最大直径
        Args:
            root: 根节点
        ...
        if not root.left and not root.right:
            return 0
        left_deep = self.search_helper(root.left) + 1 if root.left else 0
        right_deep = self.search_helper(root.right) + 1 if root.right else 0
