...
class Solution:
    ...
    def flood_fill_method(self, image: List[List[int]], sr: int, sc: int, old_color: int, new_color: int) -> List[
        List[int]]:
        '''
           数组转变
       Args:
       ...
       '''
        ...
        self.flood_fill_method(image, sr + 1, sc, old_color, new_color)
        self.flood_fill_method(image, sr, sc + 1, old_color, new_color)
        self.flood_fill_method(image, sr - 1, sc, old_color, new_color)
        self.flood_fill_method(image, sr, sc - 1, old_color, new_color)
        return image
    ...
    def flood_fill(self, image: List[List[int]], sr: int, sc: int, new_color: int) -> List[List[int]]:
        ...
        Returns:
            填充的颜色
        '''
        return self.flood_fill_method(image, sr, sc, image[sr][sc], new_color)
